...
//			
//			pstmt = con.prepareStatement(sql1); //101번의 잔액70원에서 1000원을 출금
//			pstmt.executeUpdate();
//			
//			pstmt = con.prepareStatement(sql2); //102번의 잔액30원에서 1000원을 입금 
//			pstmt.executeUpdate();
//			
...
//			String sql5 = "UPDATE account SET balance = balance - 10 WHERE no='101'"; //있는 계좌번호
//			pstmt = con.prepareStatement(sql5);//101번의 잔액70원에서 10원을 출금
//			int rowcnt1 = pstmt.executeUpdate();
//			if(rowcnt1 == 0) {
//				//con.rollback();
//				throw new SQLException("101번 계좌없음");
//			}
//			pstmt = con.prepareStatement(sql3);//777번(없는 계좌)의 10원을 입금
