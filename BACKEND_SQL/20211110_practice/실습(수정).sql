...
--5. 급여가 4000 보다 많은 사원들의 부서별 급여평균를 출력하시오. 단 급여평균은 소숫점이하 2 자리에서 반올림합니다. -12건
select department_id, round(avg(salary),1)
from employees
where salary>4000
group by department_id;

--6. 부서배치를 받지 않은 사원은 제외하고 부서별 급여평균을 출력하시오.단 급여평균은 소숫점이하 2 자리에서 반올림합니다 -- 11건
select department_id, round(avg(salary),1)
from employees
...
--8. 직책(job_title)이 'President'인 사원의 사번, 이름을 출력하시오: (NaturalJOIN 으로 표현)--1건
select employee_id,first_name
from employees
natural join jobs
where jobs.job_title = 'President';

--9. 직책(job_title)이 'President'가 아닌 사원의 사번, 이름을 출력하시오(JOIN USING 으로 표현) --106건
select employee_id,first_name
from employees
join jobs
using (job_id)
...
--10. 부서번호, 부서명, 부서별 급여총액과 평균급여를 출력하시오. 단, 급여총액이 낮은 순으로 정렬하고, 평균급여는 소수점 이하 1 자리까지 나타나도록 반올림해야 한다. --11건
...
--11. 'Seattle', 'Toronto'도시에 근무하는 사원들의 도시명,사번, 이름, 부서 ID, 부서명 을 출력하시오 --20건
...
--12. 'Seattle'을 제외한 도시의 부서별 사원수가 5 명미만인 도시명, 부서 ID, 부서명, 사원수를 출력하시오 --3건
...
--13. 부서 ID 가 50 이거나 80 인 부서에 근무하는 사원들의 직무 ID 와 직책(job_title)을 출력하시오. 결과에서 직종(job_id)의 중복은 제거하시오 --5건
...
where davies.last_name = 'Davies' and e.last_name != 'Davies';